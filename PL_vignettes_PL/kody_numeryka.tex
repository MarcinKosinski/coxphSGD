...
1  1 0.07852976      1   0   0
2  2 0.10314559      0   0   1
3  3 0.04471293      1   0   1
4  4 0.03496663      1   1   1
5  5 0.14461568      0   0   1
6  6 0.09745620      1   1   0
\end{verbatim}
...
Welcome to the RTCGA (version: 1.2.1).
\end{verbatim}
...
\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{t}\NormalTok{(}\KeywordTok{simplify2array}\NormalTok{(model_1_over_t$coefficients[-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{:}\DecValTok{6}\NormalTok{)]))) ->}\StringTok{ }\NormalTok{x1}
\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{t}\NormalTok{(}\KeywordTok{simplify2array}\NormalTok{(model_1_over_50sqrt_t$coefficients[-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{:}\DecValTok{6}\NormalTok{)]))) ->}\StringTok{ }\NormalTok{x2}
\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{t}\NormalTok{(}\KeywordTok{simplify2array}\NormalTok{(model_1_over_100sqrt_t$coefficients[-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{:}\DecValTok{6}\NormalTok{)]))) ->}\StringTok{ }\NormalTok{x3}



\KeywordTok{library}\NormalTok{(ggplot2)}
\KeywordTok{qplot}\NormalTok{(}\DecValTok{1}\NormalTok{:}\KeywordTok{length}\NormalTok{(x1$EGFR), x1$EGFR)}

\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{coef =} \NormalTok{x1$BRAF,}
           \DataTypeTok{iter =} \DecValTok{1}\NormalTok{:}\KeywordTok{length}\NormalTok{(x1$BRAF)) %>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{iter,}
           \DataTypeTok{y =} \NormalTok{coef)) +}
\StringTok{  }\KeywordTok{theme_dark}\NormalTok{() +}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{col =} \StringTok{"white"}\NormalTok{) +}
\StringTok{  }\KeywordTok{xlab}\NormalTok{(}\StringTok{'Numer iteracji'}\NormalTok{) +}
\StringTok{  }\KeywordTok{ylab}\NormalTok{(}\StringTok{'Wartość współczynnika'}\NormalTok{) +}
\StringTok{  }\KeywordTok{ggtitle}\NormalTok{(}\StringTok{'Zmiany wartości współczynnika dla genu BRAF'}\NormalTok{)}


\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{coef =} \NormalTok{x1$EGFR,}
           \DataTypeTok{iter =} \DecValTok{1}\NormalTok{:}\KeywordTok{length}\NormalTok{(x1$EGFR)) %>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{iter,}
           \DataTypeTok{y =} \NormalTok{coef)) +}
\StringTok{  }\KeywordTok{theme_dark}\NormalTok{() +}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{col =} \StringTok{"white"}\NormalTok{) +}
\StringTok{  }\KeywordTok{xlab}\NormalTok{(}\StringTok{'Numer iteracji'}\NormalTok{) +}
\StringTok{  }\KeywordTok{ylab}\NormalTok{(}\StringTok{'Wartość współczynnika'}\NormalTok{) +}
\StringTok{  }\KeywordTok{ggtitle}\NormalTok{(}\StringTok{'Zmiany wartości współczynnika dla genu EGFR'}\NormalTok{)}



\KeywordTok{library}\NormalTok{(dplyr)}
\KeywordTok{bind_rows}\NormalTok{(x1, x2, x3) ->}\StringTok{ }\NormalTok{binded}
...
\KeywordTok{apply}\NormalTok{(binded, }\DataTypeTok{MARGIN =} \DecValTok{1}\NormalTok{, function(x)\{}
  \KeywordTok{full_cox_loglik_matrix}\NormalTok{(}\KeywordTok{unlist}\NormalTok{(x), testCox_binded[, -}\DecValTok{1092}\NormalTok{], testCox_binded[,}\DecValTok{1092}\NormalTok{]) }
  \NormalTok{\})->}\StringTok{ }\NormalTok{logliks_values}

\NormalTok{logliks_values %>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{model =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"model1"}\NormalTok{, }\DecValTok{485}\NormalTok{),}
                            \KeywordTok{rep}\NormalTok{(}\StringTok{"model2"}\NormalTok{, }\DecValTok{485}\NormalTok{),}
                            \KeywordTok{rep}\NormalTok{(}\StringTok{"model3"}\NormalTok{, }\DecValTok{485}\NormalTok{))) ->}\StringTok{ }\NormalTok{d2viz}

\KeywordTok{dim}\NormalTok{(d2viz)}

\KeywordTok{library}\NormalTok{(tidyr)}
\KeywordTok{gather}\NormalTok{(d2viz, model) ->}\StringTok{ }\NormalTok{d2viz_gathered}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{installTCGA}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
