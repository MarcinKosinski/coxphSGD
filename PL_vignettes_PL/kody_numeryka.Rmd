...
as.data.frame(t(simplify2array(model_1_over_t$coefficients[-c(1:6)]))) -> x1
as.data.frame(t(simplify2array(model_1_over_50sqrt_t$coefficients[-c(1:6)]))) -> x2
as.data.frame(t(simplify2array(model_1_over_100sqrt_t$coefficients[-c(1:6)]))) -> x3



library(ggplot2)
qplot(1:length(x1$EGFR), x1$EGFR)

data.frame(coef = x1$BRAF,
           iter = 1:length(x1$BRAF)) %>%
  ggplot(aes(x = iter,
           y = coef)) +
  theme_dark() +
  ...
  ggtitle('Zmiany wartości współczynnika dla genu BRAF')


data.frame(coef = x1$EGFR,
           iter = 1:length(x1$EGFR)) %>%
  ggplot(aes(x = iter,
           y = coef)) +
  theme_dark() +
  geom_point(col = "white") +
  xlab('Numer iteracji') +
  ylab('Wartość współczynnika') +
  ggtitle('Zmiany wartości współczynnika dla genu EGFR')



library(dplyr)
bind_rows(x1, x2, x3) -> binded
...
apply(binded, MARGIN = 1, function(x){
  full_cox_loglik_matrix(unlist(x), testCox_binded[, -1092], testCox_binded[,1092]) 
  })-> logliks_values

logliks_values %>%
  mutate(model = c(rep("model1", 485),
                            rep("model2", 485),
                            rep("model3", 485))) -> d2viz

dim(d2viz)

library(tidyr)
gather(d2viz, model) -> d2viz_gathered
