...
do.call(rbind,model_1_over_100sqrt_t$coefficients) %>% as.data.frame -> model3

#do.call(rbind,testCox) -> testCox_binded

library(pbapply)
pbapply(model3, MARGIN = 1, function(x){
  partial_coxph_loglik(testCox_binded[,-1093], beta = x, status_number = 1092)
}) -> cum_log_lik_model3

save(cum_log_lik_model3, file = "cum_log_lik_model3.rda")
