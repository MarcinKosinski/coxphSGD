...
Poniższymi wywołaniami kodów z pakietu $\mathcal{R}$ \cite{programikr} można wygenerować 1000 obserwacji z rozkładu normalnego i na ich podstawie wygenerować 1000 zmiennych z rozkładu dwupunktowego o takim rozkładzie prawdopodobieństwa sukcesu, by rzeczywiste współczynniki w modelu regresji logistycznej dla tych zmiennych wynosiły odpowiednio: 2 dla wyrazu wolnego oraz 3 dla zmiennej objaśniającej z rozkładu normalnego. 
\begin{Shaded}
\begin{Highlighting}[]
...
Na każdym z wykresów na osi $OX$ ukazano współczynnik dla zmiennej objaśniającej w prostym modelu regresji logistycznej z jedną zmienną objaśniającą w kolejnych krokach poszczególnych algorytmów. Na osi $OY$ zaznaczono współczynnik wyrazu wolnego w tym modelu. Czarną kropką zaznaczono prawdziwe ekstremum funkcji log-wiarogodności w tym modelu, czyli punkt $(x,y) = (3,2)$. Trajektoria zbieżności do minimum dla odrębnych algorytmów zaznaczono oddzielnymi kolorami, które są wyjaśnione na legendzie wykresu, na której dodatkowo wpisano liczbę kroków wymaganą przez algorytm do zbieżności. Przez \texttt{GDI} oznaczono trajektoria dla algorytmu spadku gradientu rzędu I, przez \texttt{GDII} oznaczono trajektoria dla algorytmu spadku gradientu rzędu II, zaś przez \texttt{SGD.i} oznaczono 6 różnych trajektorii dla algorytmów stochastycznego spadku gradientu dla różnych ciągów odpowiadających długości kroku algorytmu. Indeks $i$ odpowiada ciągowi wybranemu do wyznaczania długości kroku algorytmu na zasadzie $\alpha_{ki} = \frac{i}{k}$.

W trakcie każdej symulacji postawiono pewne warunki konieczne do zbieżności. Ustalono maksymalną liczbę iteracji na 5001 dla algorytmów spadku gradientu i 1001 dla wariantów stochastycznych, gdzie przez pierwszy krok rozumiano start z punktu startowego. Dodatkowo warunek stopu ustalono na $\epsilon=0.00001$ oraz ustalono ciąg odpowiadający długościom kroków w algorytmie spadku gradientu rzędu I na $\alpha_{k} = \frac{1}{100k}$. 

Symulacje powtórzono czterokrotnie dla czterech różnych punktów startu algorytmów $$\beta_0 = (0,0), (1,2), (4,3), (3.1,2.1).$$

Ponieważ każda trajektoria w procesie estymacji metodą stochastycznego gradientu dla tych samych parametrów zbieżności jest inna, z racji na losową kolejność obserwacji, toteż dla każdego z czterech ustalonych punktów startowych zdecydowano się zobrazować symulację dwukrotnie, aby móc przedstawić stochastyczny aspekt tej metody.

\begin{figure}[h]
  %\vspace{-10pt}
  \begin{center}
   \subfigure[Symulacja dla $\beta_0 = (0,0)$ nr 1.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_00_1.pdf}}
   \subfigure[Symulacja dla $\beta_0 = (0,0)$ nr 2.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_00_2.pdf}}
...
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot06.pdf}}
  \end{center}
  %\vspace{-10pt}
  \caption{\label{fig:scasd}Porównanie algorytmów spadku gradientu dla punktu startowego $\beta_0 = (0,0)$.}
\end{figure}
...
\begin{figure}[h]
  %\vspace{-10pt}
  \begin{center}
   \subfigure[Symulacja dla $\beta_0 = (1,2)$ nr 1.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_2_1_1.pdf}}
   \subfigure[Symulacja dla $\beta_0 = (1,2)$ nr 2.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_2_1_2.pdf}}
%   \subfigure[Symulacja 3.]{
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot03.pdf}}
%   \subfigure[Symulacja 4.]{
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot04.pdf}}
...
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot06.pdf}}
  \end{center}
  %\vspace{-10pt}
  \caption{\label{fig:sc2}Porównanie algorytmów spadku gradientu dla punktu startowego $\beta_0 = (1,2)$.}
\end{figure}
...
\begin{figure}[h]
  %\vspace{-10pt}
  \begin{center}
   \subfigure[Symulacja dla $\beta_0 = (4,3)$ nr 1.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_3_4_1.pdf}}
   \subfigure[Symulacja dla $\beta_0 = (4,3)$ nr 2.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_3_4_2.pdf}}
%   \subfigure[Symulacja 3.]{
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot03.pdf}}
%   \subfigure[Symulacja 4.]{
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot04.pdf}}
...
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot06.pdf}}
  \end{center}
  %\vspace{-10pt}
  \caption{\label{fig:sc3}Porównanie algorytmów spadku gradientu dla punktu startowego $\beta_0 = (4,3)$.}
\end{figure}
...
\begin{figure}[h]
  %\vspace{-10pt}
  \begin{center}
   \subfigure[Symulacja dla $\beta_0 = (3.1,2.1)$ nr 1.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_21_31_1.pdf}}
   \subfigure[Symulacja dla $\beta_0 = (3.1,2.1)$ nr 2.]{
     \includegraphics[width=0.9\textwidth, height=300pt]{Obrazki/sgd_21_31_2.pdf}}
%   \subfigure[Symulacja 3.]{
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot03.pdf}}
%   \subfigure[Symulacja 4.]{
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot04.pdf}}
...
%     \includegraphics[width=0.32\textwidth]{Obrazki/Rplot06.pdf}}
  \end{center}
  %\vspace{-10pt}
  \caption{\label{fig:sc4asd}Porównanie algorytmów spadku gradientu dla punktu startowego $\beta_0 = (3.1,2.1)$.}
\end{figure}
