...
gdzie
\begin{equation}
L_p(\beta) = \prod\limits_{i=1}^{n}f(t_j;\beta)^{\delta_j}S(t_j;\beta)^{1-\delta_j}=\prod\limits_{i=1}^{n}\lambda(t_j;\beta)^{\delta_j}S(t_j;\beta)
\end{equation}
...
\end{equation}
gdzie sumowanie jest po $l\in \mathscr{R}(t_i)$, zaś czynnik
\begin{equation}
