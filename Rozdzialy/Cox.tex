...
W analizie przeżycia, w której model Coxa znalazł największe zastosowania, ogromnie popularna jest funkcja hazardu.
\begin{definition}
...
U_k(\beta)=\dfrac{\partial\ell_k(\beta)}{\partial\beta_k}=\sum\limits_{i=1}^{K}\Big(X_{ik}-\dfrac{\sum X_{ik} e^{X_l'\beta}}{\sum e^{X_l'\beta}}\Big),
\end{equation}
gdzie sumowanie jest po $l\in \mathscr{R}(t_i)$. Czynnik
\begin{equation}
A_{ik} = \dfrac{\sum X_{ik} e^{X_l'\beta}}{\sum e^{X_l'\beta}}
\end{equation}
...
gdzie
\begin{equation}
C_{ik_1k_2}(\beta)=\dfrac{\sum X_{lk_1}X_{l_k2}e^{X_l'\beta}}{\sum e^{X_l'\beta}} - A_{ik_1}(\beta)A_{i_k2}(\beta)
\end{equation}
to kowariancja pomiędzy $X_{.k_1}$ ($k_1$-tymi zmiennymi) a $X_{.k_2}$ ($k_2$-tymi zmiennymi) przy tej formie ważonego próbkowania.

\textcolor{red}{Czy wplatać tę informację: \cite{cox} "To calculate the expected value of \ref{grad} it would be necessary to know the times at which individuals who failed would have been censored had the not failed. This information would often not be available and in any case might well be thought irrelevenat; this point is connected with difficulties of conditionality at the basis of a sampling theory approach to statistics(Pratt,1962). here we shall use asymptotic arguments in which \ref{grad} can be used directly for the estimation of variances, $\beta$ being replaced bu a suitable estimate.}
